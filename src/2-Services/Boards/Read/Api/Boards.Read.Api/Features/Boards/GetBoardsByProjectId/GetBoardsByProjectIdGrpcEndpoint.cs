...
namespace TaskoMask.Services.Boards.Read.Api.Features.Boards.GetBoardsByProjectId
{
    ...
    public class GetBoardsByProjectIdGrpcEndpoint : GetBoardsByProjectIdGrpcService.GetBoardsByProjectIdGrpcServiceBase
    {
        ...
        public override async Task Handle(GetBoardsByProjectIdGrpcRequest request, IServerStreamWriter<GetBoardsByProjectIdGrpcResponse> responseStream, ServerCallContext context)
        {
            ...
            foreach (var board in boards.Value)
                await responseStream.WriteAsync(_mapper.Map<GetBoardsByProjectIdGrpcResponse>(board));
        }
    }
