...
public class UpdateCardTests : TestsBaseFixture
{
    ...
    public async Task Card_is_updated()
    {
        ...
        //Act
        var result = await updateCardUseCase.Handle(updateCardRequest, CancellationToken.None);
    ...
    public async Task Updating_a_card_will_throw_an_exception_if_Id_is_not_existed()
    {
        ...
        //Act
        Func<Task> act = async () => await updateCardUseCase.Handle(updateCardRequest, CancellationToken.None);
    ...
    {
        updateCardUseCase = new UpdateCardUseCase(BoardAggregateRepository, MessageBus, InMemoryBus);
    }
