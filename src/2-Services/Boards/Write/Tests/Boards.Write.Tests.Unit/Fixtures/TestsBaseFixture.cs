...
namespace TaskoMask.Services.Boards.Write.Tests.Unit.Fixtures
{
    public abstract class TestsBaseFixture : UnitTestsBase
    {
        ...
        private void CommonFixtureSetup()
        {
            ...
            BoardAggregateRepository.GetByIdAsync(Arg.Is<string>(x => Boards.Any(o => o.Id == x))).Returns(args => Boards.First(u => u.Id == (string)args[0]));
            BoardAggregateRepository.AddAsync(Arg.Any<Board>()).Returns(args => { Boards.Add((Board)args[0]); return Task.CompletedTask; });
            BoardAggregateRepository.UpdateAsync(Arg.Is<Board>(x => Boards.Any(o => o.Id == x.Id))).Returns(args =>
            {
