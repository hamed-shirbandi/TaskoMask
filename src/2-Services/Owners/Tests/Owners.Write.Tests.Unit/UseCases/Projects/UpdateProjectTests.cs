...
public class UpdateProjectTests : TestsBaseFixture
{
    ...
    public async Task Project_is_updated()
    {
        ...
        //Act
        var result = await updateProjectUseCase.Handle(updateProjectRequest, CancellationToken.None);
    ...
    public async Task Updating_an_project_will_throw_an_exception_if_Id_is_not_existed()
    {
        ...
        //Act
        Func<Task> act = async () => await updateProjectUseCase.Handle(updateProjectRequest, CancellationToken.None);
    ...
    {
        updateProjectUseCase = new UpdateProjectUseCase(OwnerAggregateRepository, MessageBus, InMemoryBus);
    }
