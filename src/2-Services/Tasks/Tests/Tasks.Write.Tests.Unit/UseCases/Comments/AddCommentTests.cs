...
namespace TaskoMask.Services.Tasks.Write.Tests.Unit.UseCases.Comments
{
    public class AddCommentTests : TestsBaseFixture
    {
        ...
        public async Task Comment_is_added()
        {
            ...
            //Act
            var result = await _addCommentUseCase.Handle(addCommentRequest, CancellationToken.None);
        ...
        public async Task Adding_a_comment_will_throw_an_exception_if_task_Id_is_not_existed()
        {
            ...
            //Act
            Func<Task> act = async () => await _addCommentUseCase.Handle(addCommentRequest, CancellationToken.None);
        ...
        {
            _addCommentUseCase = new AddCommentUseCase(TaskAggregateRepository, MessageBus, InMemoryBus);
        }
