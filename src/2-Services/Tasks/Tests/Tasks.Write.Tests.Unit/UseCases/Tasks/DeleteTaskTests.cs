...
namespace TaskoMask.Services.Tasks.Write.Tests.Unit.UseCases.Tasks
{
    public class DeleteTaskTests : TestsBaseFixture
    {
        ...
        public async Task Task_is_deleted()
        {
            ...
            //Act
            var result = await _deleteTaskUseCase.Handle(deleteTaskRequest, CancellationToken.None);
        ...
        public async Task Deleting_a_task_will_throw_an_exception_if_Id_is_not_existed()
        {
            ...
            //Act
            Func<Task> act = async () => await _deleteTaskUseCase.Handle(deleteTaskRequest, CancellationToken.None);
        ...
        {
            _deleteTaskUseCase = new DeleteTaskUseCase(TaskAggregateRepository, MessageBus, InMemoryBus);
        }
